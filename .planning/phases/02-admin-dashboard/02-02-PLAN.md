...
must_haves:
  truths:
    ...
    - "Admin role validation enforced at database RPC layer (is_admin() from Phase 1)"
  ...
  artifacts:
    - path: "web/app/admin/analytics/page.tsx"
      provides: "Server Component that fetches analytics and validates admin via RPC"
  ...
  key_links:
    - from: "web/app/admin/analytics/page.tsx"
      to: "analytics RPC functions (Phase 1)"
      via: "supabase.rpc() calls - RPC functions enforce is_admin() check"
      pattern: "rpc\\('get_"
      note: "Security enforced at DB layer: RPC raises 'Access denied' for non-admin users"
    - from: "web/app/admin/analytics/page.tsx"
      to: "web/app/admin/forbidden.tsx"
      via: "forbidden() call when RPC returns 'Access denied' error"
      pattern: "forbidden\\(\\)"
      note: "RPC-based validation is intentional - database is source of truth for admin role"
...
**Security Model:** Admin access is enforced at the database layer via Phase 1's is_admin() function. The page does NOT perform a separate client-side role check - instead it calls RPC functions that enforce admin-only access. If the user lacks admin role, the RPC returns "Access denied" error, which triggers forbidden(). This is intentional: the database is the single source of truth for authorization.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT - Admin Validation Pattern:**
    The page validates admin access by checking if get_summary_stats() returns "Access denied" error.
    This is the intentional pattern because:
    - Phase 1 RPC functions enforce is_admin() at database level (see 01-02-SUMMARY.md)
    - Database is the single source of truth for user roles
    - No client-side role check needed - RPC layer is the security boundary
    - If user is not admin, RPC raises EXCEPTION which becomes error response
    - Page catches this error and calls forbidden() to show 403 page
  </action>
  <verify>
    ...
    - File contains comment explaining RPC-based admin validation pattern
  </verify>
  <done>
    - Server Action for manual refresh exists
    - Analytics page fetches data via RPC and renders components
    - Admin validation enforced via RPC layer (is_admin() in database)
    - Page calls forbidden() when RPC returns "Access denied" error
  </done>
</task>
...
5. **Admin validation pattern verify:**
   ```bash
   grep -A5 "ADMIN VALIDATION" web/app/admin/analytics/page.tsx
   # Should show comment explaining RPC-based validation
   ```
</verification>

<success_criteria>
- /admin/analytics page exists and renders (for admin users)
- Non-admin authenticated users see 403 page (via RPC error -> forbidden())
- Admin role validation enforced at database RPC layer (is_admin() from Phase 1)
