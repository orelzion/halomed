...
The Scheduling Agent is responsible for the server-side track scheduling logic, including the `generate-schedule` Edge Function, Jewish calendar integration, and the 14-day rolling window generation.
...
- **Coordinates with**: Backend Agent (database), Content Generation Agent (content for units)
- **Outputs to**: Sync Agent (scheduled units for sync)
...
**Requirements:**
- Use Hebrew calendar library (e.g., @hebcal/core)
- Check for major holidays that exclude study
- Holidays to exclude: Rosh Hashana, Yom Kippur, Sukkot, Shmini Atzeret, Simchat Torah, Pesach, Shavuot

**Implementation Pattern:**
- Convert date to Hebrew date
- Get holidays for that date
- Check if any major holidays match
typescript
...
**Requirements:**
- Iterate dates within window (start_date to start_date + days_ahead)
- For each date, check if allowed by schedule_type
- If allowed, assign next content reference and create user_study_log row
- Track user's current position in track content
- Ensure content_cache exists for each reference (trigger generation if needed)

**Implementation Pattern:**
- Use date iteration loop
- Check schedule type rules (weekday, holiday exclusion)
- Use UPSERT pattern to avoid duplicates
- Increment content index for sequential assignment
typescript
...
function isScheduledDay(date: Date, scheduleType: string): boolean {
  switch (scheduleType) {
    case 'DAILY_WEEKDAYS_ONLY':
      ...
      // Skip Saturday (6) and Friday after sunset (handled by isJewishHoliday)
...
```
Content_{Tractate}.{Chapter}.{Mishnah}
```

Example: `Content_Berakhot.1.1`
...
**Key Queries:**
- Get user's current position: COUNT completed units for user/track
- Create scheduled unit: INSERT with ON CONFLICT DO NOTHING
- Check content exists: SELECT from content_cache by ref_id
...
- Unit tests for Jewish calendar integration
