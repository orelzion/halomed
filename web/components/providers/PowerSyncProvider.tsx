...
export function PowerSyncProvider({ children }: PowerSyncProviderProps) {
  ...
  const upsertTodayLogsFallback = useCallback(async (
    tracks: Array<{ id: string; start_date: string | null }>,
    today: string,
    userId: string
  ) => {
   ...
   */
  const generateSchedule = useCallback(async (forceSync = false) => {
    if (!session) {
  ...
  // Then relies on visibility change and periodic sync for subsequent runs
  useEffect(() => {
    if (!session) {
      return;
    }
  ...
  // Fix #1 & #2: Visibility change listener for PWA resume
  // When app returns from background, check if sync is needed
  useEffect(() => {
    ...
    // Handle online event - sync when coming back online
    const handleOnline = () => {
  ...
  // Fix #4: Periodic sync while app is open (every 5 minutes)
  // This ensures content stays fresh even if the app stays open for a long time
  useEffect(() => {
    if (!session) {
      return;
    }
  ...
  return (
    <PowerSyncContext.Provider value={{ isConnected, hasSynced, error, triggerSync }}>
      {children}
