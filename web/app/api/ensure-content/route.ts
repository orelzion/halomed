...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${supabaseUrl}/functions/v1/ensure-content`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    return NextResponse.json(
      { error: 'Content check failed', details: errorDetails.error || errorDetails.details || responseBody },
