...
Deno.test('scheduling: Sunday is not a scheduled day (weekdays only)', () => {
  const sunday = new Date('2024-01-21'); // Sunday, Jan 21, 2024
  const result = isScheduledDay(sunday, 'DAILY_WEEKDAYS_ONLY');
...
Deno.test('scheduling: Tuesday is a scheduled day', () => {
  const tuesday = new Date('2024-01-16'); // Tuesday, Jan 16, 2024
  const result = isScheduledDay(tuesday, 'DAILY_WEEKDAYS_ONLY');
  assertEquals(result, true, 'Tuesday should be scheduled');
});

Deno.test('scheduling: Wednesday is a scheduled day', () => {
  const wednesday = new Date('2024-01-17'); // Wednesday, Jan 17, 2024
  const result = isScheduledDay(wednesday, 'DAILY_WEEKDAYS_ONLY');
  assertEquals(result, true, 'Wednesday should be scheduled');
});

Deno.test('scheduling: Thursday is a scheduled day', () => {
  const thursday = new Date('2024-01-18'); // Thursday, Jan 18, 2024
  const result = isScheduledDay(thursday, 'DAILY_WEEKDAYS_ONLY');
  assertEquals(result, true, 'Thursday should be scheduled');
});

Deno.test('scheduling: Friday is a scheduled day', () => {
  const friday = new Date('2024-01-19'); // Friday, Jan 19, 2024
  const result = isScheduledDay(friday, 'DAILY_WEEKDAYS_ONLY');
...
Deno.test('scheduling: regular weekday (Monday) is not a holiday', () => {
...
Deno.test('scheduling: weekday that is a holiday is not scheduled', () => {
  // Passover 2024 first day is a Monday (weekday but holiday)
  const passoverMonday = new Date('2024-04-22'); // Monday, but Passover
  const result = isScheduledDay(passoverMonday, 'DAILY_WEEKDAYS_ONLY');
  assertEquals(result, false, 'Weekday that is a holiday should not be scheduled');
});

Deno.test('scheduling: weekday that is not a holiday is scheduled', () => {
  const regularMonday = new Date('2024-06-17'); // Regular Monday
  const result = isScheduledDay(regularMonday, 'DAILY_WEEKDAYS_ONLY');
  assertEquals(result, true, 'Regular weekday should be scheduled');
});

Deno.test('scheduling: Saturday that is not a holiday is not scheduled', () => {
  const regularSaturday = new Date('2024-06-15'); // Regular Saturday
  const result = isScheduledDay(regularSaturday, 'DAILY_WEEKDAYS_ONLY');
